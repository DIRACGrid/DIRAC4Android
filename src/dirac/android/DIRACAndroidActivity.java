...
public class DIRACAndroidActivity extends SherlockActivity implements ActionBar.OnNavigationListener {
	...
	case R.id.menu_refresh:
		...
		Integer mymax = 1;
		mymax = CacheHelper.readInteger(getBaseContext(), CacheHelper.NMAXBJOBS, mymax);	
		if(JobType == ""){
			apiCall.getSummaryandJobs(Constants.REQUEST_SUMMARY,Constants.REQUEST_JOBS+"?maxJobs="+mymax.toString());
		}else{
			apiCall.getSummaryandJobs(Constants.REQUEST_SUMMARY+"?"+JobType,Constants.REQUEST_JOBS+"?maxJobs="+mymax.toString()+"&"+JobType);
